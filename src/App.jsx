...
export default function App() {
  ...
  const [careerForm, setCareerForm] = useState({ company:"", period:"", role:"", description:"" });
  ...
  const handleSaveCareer = async () => {
    if (!careerForm.company || !careerForm.period) { showToast("企業名・在職期間は必須です"); return; }
    try {
      if (editingCareerId) {
        await updateDoc(doc(db, "profiles", user.email, "careers", editingCareerId), { ...careerForm });
        showToast("経歴を更新しました");
      } else {
        await addDoc(collection(db, "profiles", user.email, "careers"), { ...careerForm, order: careers.length, createdAt: serverTimestamp() });
        showToast("経歴を追加しました");
      }
      setCareerForm({ company:"", period:"", role:"", description:"" });
      setEditingCareerId(null);
    } catch(e) { showToast("保存に失敗しました"); }
  };

  const handleEditCareer = (career) => {
    setCareerForm({ company: career.company||"", period: career.period||"", role: career.role||"", description: career.description||"" });
          ...
          <div className="mypage-wrap">
            ...
            <div className="mypage-card">
              ...
              <div className="career-form">
                ...
                <div className="mypage-grid" style={{marginBottom:12}}>
                  <div>
                  ...
                  <div>
                    ...
                    <input className="mypage-input" value={careerForm.period} onChange={e=>setCareerForm({...careerForm,period:e.target.value})} placeholder="例：2020年4月〜2024年3月" />
                  </div>
                </div>
                ...
                <div style={{display:"flex",gap:10}}>
                  ...
                  {editingCareerId && <button style={{padding:"12px 24px",borderRadius:100,background:"var(--gray200)",border:"none",cursor:"pointer",fontWeight:600}} onClick={()=>{setCareerForm({company:"",period:"",role:"",description:""});setEditingCareerId(null);}}>キャンセル</button>}
                </div>
              </div>
            </div>
          </div>
