...
export default function App() {
  ...
  const [adminForm, setAdminForm] = useState({
    title: "", company: "", type: "業務委託", rate: "", location: "",
    tags: "", remote: false, urgent: false, highPay: false, lowExp: false,
    description: "", requirements: "", period: "", published: true
  });
  ...
  // Firestoreから案件をリアルタイム取得
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "jobs"), (snapshot) => {
      if (!snapshot.empty) {
        const firestoreJobs = snapshot.docs
  ...
  const getErrorMessage = (code) => {
    switch(code) {
      ...
      case "auth/too-many-requests": return "ログイン試行が多すぎます。しばらくしてからお試しください";
      default: return "エラーが発生しました。もう一度お試しください";
    }
  };

  const handleRegister = async () => {
    console.log("handleRegister 開始");
    setAuthError("");
    ...
    try {
      console.log("Firebase登録開始:", regForm.email);
      ...
      console.log("Firebase登録成功！EmailJS送信開始");
      ...
      try {
        ...
        console.log("EmailJS結果:", res.status, text);
      } catch(err) {
        console.error("EmailJS fetch エラー:", err);
      }

      showToast("登録が完了しました！");
    } catch (e) {
      console.error("Firebase登録エラー:", e.code, e.message);
      setAuthError(getErrorMessage(e.code));
    }
  };
  ...
  const handleAddJob = async () => {
    if (!adminForm.title || !adminForm.company || !adminForm.rate) {
      showToast("タイトル・企業名・報酬は必須です");
      return;
    }
  ...
  const handleEditJob = (job) => {
    setAdminForm({
      title: job.title || "", company: job.company || "", type: job.type || "業務委託",
      rate: job.rate || "", location: job.location || "",
      tags: Array.isArray(job.tags) ? job.tags.join(", ") : (job.tags || ""),
      remote: job.remote || false, urgent: job.urgent || false,
      highPay: job.highPay || false, lowExp: job.lowExp || false,
      description: job.description || "", requirements: job.requirements || "",
      period: job.period || "", published: job.published !== false,
    });
    setEditingJob(job);
  };
  ...
  const handleTogglePublish = async (job) => {
    try {
      await updateDoc(doc(db, "jobs", job.id), { published: !job.published });
      showToast(job.published ? "非公開にしました" : "公開しました");
    } catch(e) {
      showToast("更新に失敗しました");
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, "applications"), (snapshot) => {
      ...
      setApplications(apps.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)));
    });
    return () => unsubscribe();
  }, [isAdmin]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      collection(db, "applications", selectedApplication.id, "messages"),
      (snapshot) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, "applications"), (snapshot) => {
      const myApps = snapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() }))
        .filter(app => app.userEmail === user.email);
      setMyApplications(myApps);
      setAppliedJobs(myApps.map(a => a.jobId));
    });
    return () => unsubscribe();
  }, [user, isAdmin]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      try {
        await fetch("https://api.emailjs.com/api/v1.0/email/send", {
          ...
          body: JSON.stringify({
            ...
            user_id: "l-4JMsbbRt5ETL0Su",
            template_params: {
              name: `【応募通知】${user.name}さんが「${selectedJob.title}」に応募しました`,
              email: user.email,
            }
          })
        });
      ...
      showToast("応募に失敗しました。もう一度お試しください。");
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await addDoc(collection(db, "applications", selectedApplication.id, "messages"), {
        text: newMessage,
  ...
  if (authLoading) {
    return <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100vh',fontFamily:'sans-serif',color:'#8a8a7a'}}>読み込み中...</div>;
  }
  ...
  const css = `
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .admin-card h3 { font-size: 16px; font-weight: 700; margin-bottom: 20px; color: var(--ink); }
    ...
    .admin-input {
      ...
      font-size: 14px; font-family: 'Noto Sans JP', sans-serif;
      background: var(--gray100); outline: none;
    }
    ...
    .admin-textarea:focus { border-color: var(--accent); background: white; }
    .admin-select {
      width: 100%; padding: 10px 14px;
      border: 1.5px solid var(--gray200); border-radius: 8px;
      font-size: 14px; background: var(--gray100); outline: none;
    }
    ...
    .admin-btn.secondary {
      background: var(--gray200); color: var(--gray700); margin-top: 8px;
    }
    .admin-job-list { display: flex; flex-direction: column; gap: 12px; max-height: 600px; overflow-y: auto; }
    .admin-job-item {
      padding: 16px; border-radius: 10px;
      border: 1px solid var(--gray200); background: var(--gray100);
      display: flex; align-items: center; justify-content: space-between; gap: 12px;
    }
    .admin-job-info { flex: 1; }
    .admin-job-title { font-size: 14px; font-weight: 700; color: var(--ink); margin-bottom: 4px; }
    ...
    .admin-action-btn.publish { background: var(--green-soft); color: var(--green); }
    .admin-action-btn.unpublish { background: var(--gray200); color: var(--gray500); }
    .published-badge {
      font-size: 11px; padding: 2px 8px; border-radius: 100px;
      font-weight: 600;
    }
    .published-badge.on { background: var(--green-soft); color: var(--green); }
    .published-badge.off { background: var(--gray200); color: var(--gray500); }
    /* Messaging */
    .msg-overlay {
      position: fixed; inset: 0;
    ...
    .msg-header-title { font-size: 15px; font-weight: 700; }
    .msg-header-sub { font-size: 12px; color: var(--gray500); margin-top: 2px; }
    .msg-body { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 12px; }
    .msg-bubble {
    ...
    .msg-bubble.theirs {
      background: var(--gray100); color: var(--ink);
      align-self: flex-start; border-bottom-left-radius: 4px;
    }
    .msg-bubble-name { font-size: 11px; font-weight: 600; margin-bottom: 4px; opacity: 0.7; }
    .msg-footer {
      padding: 16px 20px; border-top: 1px solid var(--gray200);
      display: flex; gap: 10px;
    }
    .msg-input {
      flex: 1; padding: 10px 16px; border-radius: 100px;
      border: 1.5px solid var(--gray200); outline: none; font-size: 14px;
      font-family: 'Noto Sans JP', sans-serif;
    }
    ...
    /* Application list in admin */
    .app-tabs { display: flex; gap: 0; margin-bottom: 20px; border-bottom: 2px solid var(--gray200); }
    .app-tab {
      padding: 10px 20px; font-size: 14px; font-weight: 600;
      color: var(--gray500); cursor: pointer; border-bottom: 2px solid transparent;
      margin-bottom: -2px;
    }
    ...
    .app-item:hover { border-color: var(--accent); box-shadow: var(--shadow-sm); }
    .app-item-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px; }
    .app-item-title { font-size: 14px; font-weight: 700; }
    .app-item-meta { font-size: 12px; color: var(--gray500); }
    .status-badge {
      font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 100px;
    }
    .status-badge.未対応 { background: #fef3c7; color: #b45309; }
    .status-badge.対応中 { background: var(--blue-soft); color: var(--blue); }
    .status-badge.採用 { background: var(--green-soft); color: var(--green); }
    .status-badge.不採用 { background: var(--red-soft); color: var(--red); }

    /* My applications page */
    .myapp-page { min-height: 100vh; background: var(--gray100); }
  `;

  // --- Admin ---
  if (page === "admin") {
    if (!isAdmin) {
      return <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100vh',fontFamily:'sans-serif'}}>アクセス権限がありません</div>;
    }
    const [adminTab, setAdminTab] = [useState("jobs")].flat();
    return (
      <>
        <style>{css}</style>
        <div className="admin-page">
          ...
          <div className="admin-body">
            <AdminContent
              ...
              editingJob={editingJob} adminLoading={adminLoading}
              handleAddJob={handleAddJob} resetAdminForm={resetAdminForm}
              handleEditJob={handleEditJob} handleDeleteJob={handleDeleteJob}
              handleTogglePublish={handleTogglePublish} jobs={jobs}
              ...
              handleSendMessage={handleSendMessage} user={user}
              showMessages={showMessages} setShowMessages={setShowMessages}
            />
          </div>
        </div>
      ...
      <>
        ...
        <div className="auth-page">
          <div className="auth-card">
            ...
            {authError && <div style={{background:'#fef2f2',color:'#dc2626',padding:'12px 16px',borderRadius:8,fontSize:13,marginBottom:16,border:'1px solid #fecaca'}}>{authError}</div>}
      ...
      <>
        ...
        <div className="myapp-page">
          <nav className="dash-nav">
            <div className="logo" style={{color:"var(--ink)"}} onClick={() => setPage("dashboard")}>
              <div className="logo-mark">S</div>SalesBoard
            </div>
            <button onClick={() => setPage("dashboard")} style={{padding:'8px 20px',borderRadius:100,border:'1.5px solid var(--gray200)',background:'white',cursor:'pointer',fontSize:14,fontWeight:600}}>← ダッシュボードへ</button>
          </nav>
          <div style={{maxWidth:800,margin:'0 auto',padding:32}}>
            <h2 style={{fontSize:22,fontWeight:900,marginBottom:24}}>📨 応募履歴・メッセージ</h2>
            {myApplications.length === 0 && (
              <div style={{textAlign:'center',padding:60,color:'var(--gray500)'}}>
                <div style={{fontSize:40,marginBottom:12}}>📭</div>
                <div style={{fontSize:16,fontWeight:600}}>まだ応募した案件がありません</div>
              </div>
            )}
            {myApplications.map(app => (
              <div key={app.id} className="app-item" style={{marginBottom:12}}>
                <div className="app-item-top">
                  <div className="app-item-title">{app.jobTitle}</div>
                  <span className={`status-badge ${app.status}`}>{app.status}</span>
                </div>
                <div className="app-item-meta">{app.company} ・ 応募日: {app.createdAt?.toDate?.()?.toLocaleDateString?.('ja-JP') || ""}</div>
                <div style={{marginTop:10}}>
                  <button style={{padding:'6px 16px',borderRadius:100,fontSize:12,fontWeight:600,cursor:'pointer',border:'none',background:'var(--blue-soft)',color:'var(--blue)'}}
                    onClick={() => { setSelectedApplication(app); setShowMessages(true); }}>
                    💬 担当者とメッセージ
                  </button>
                </div>
              </div>
        ...
        {/* Messaging Modal for User */}
        {showMessages && selectedApplication && (
          <div className="msg-overlay" onClick={() => setShowMessages(false)}>
            <div className="msg-panel" onClick={e => e.stopPropagation()}>
              <div className="msg-header">
                <div>
                  <div className="msg-header-title">💬 {selectedApplication.jobTitle}</div>
                  <div className="msg-header-sub">担当者とのメッセージ</div>
                </div>
                <button onClick={() => setShowMessages(false)} style={{background:'none',border:'none',fontSize:20,cursor:'pointer',color:'var(--gray500)'}}>✕</button>
              </div>
              <div className="msg-body">
                {selectedApplication.message && (
                  <div className="msg-bubble mine">
                    <div className="msg-bubble-name">あなた（応募時のメッセージ）</div>
                    {selectedApplication.message}
                  </div>
                )}
                {messages.map(msg => (
                  <div key={msg.id} className={`msg-bubble ${msg.senderEmail === user.email ? 'mine' : 'theirs'}`}>
                    <div className="msg-bubble-name">{msg.senderName}</div>
                    {msg.text}
                  </div>
                ))}
              </div>
              <div className="msg-footer">
                <input className="msg-input" placeholder="メッセージを入力..."
                  value={newMessage} onChange={e => setNewMessage(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && handleSendMessage()} />
                <button className="msg-send-btn" onClick={handleSendMessage}>送信</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="user-pill" onClick={() => setMenuOpen(!menuOpen)}>
              ...
              {menuOpen && (
                <div className="dropdown">
                  <div className="dropdown-item" onClick={() => { setPage("myapps"); setMenuOpen(false); }}>📨 応募履歴・メッセージ</div>
                  <div className="dropdown-item">プロフィール編集</div>
                  {isAdmin && (
                    <div className="dropdown-item" onClick={() => { setPage("admin"); setMenuOpen(false); }} style={{color:'var(--accent)',fontWeight:700}}>
                      ⚙️ 管理者画面
                    </div>
                  )}
        ...
        <div className="msg-overlay" onClick={() => setShowMessages(false)}>
          <div className="msg-panel" onClick={e => e.stopPropagation()}>
            <div className="msg-header">
              <div>
                <div className="msg-header-title">💬 {selectedApplication.jobTitle}</div>
                <div className="msg-header-sub">{isAdmin ? `応募者: ${selectedApplication.userName}` : "担当者とのメッセージ"}</div>
              </div>
              <button onClick={() => setShowMessages(false)} style={{background:'none',border:'none',fontSize:20,cursor:'pointer',color:'var(--gray500)'}}>✕</button>
            </div>
            <div className="msg-body">
              ...
              {selectedApplication.message && (
                <div className="msg-bubble theirs">
                  <div className="msg-bubble-name">{selectedApplication.userName}（応募時のメッセージ）</div>
                  {selectedApplication.message}
                </div>
              )}
              {messages.map(msg => (
              ...
              {messages.length === 0 && !selectedApplication.message && (
                <div style={{textAlign:'center',color:'var(--gray500)',padding:40,fontSize:14}}>
...
// 管理者画面コンテンツコンポーネント
function AdminContent({ adminForm, setAdminForm, editingJob, adminLoading, handleAddJob, resetAdminForm, handleEditJob, handleDeleteJob, handleTogglePublish, jobs, applications, handleUpdateApplicationStatus, selectedApplication, setSelectedApplication, messages, newMessage, setNewMessage, handleSendMessage, user, showMessages, setShowMessages }) {
  const [adminTab, setAdminTab] = useState("jobs");

  return (
    <div>
      <div className="app-tabs">
        <div className={`app-tab ${adminTab === 'jobs' ? 'active' : ''}`} onClick={() => setAdminTab('jobs')}>
          📋 案件管理
        </div>
        <div className={`app-tab ${adminTab === 'applications' ? 'active' : ''}`} onClick={() => setAdminTab('applications')}>
          📨 応募管理 {applications.filter(a => a.status === "未対応").length > 0 && `(${applications.filter(a => a.status === "未対応").length})`}
        </div>
      </div>
      ...
      {adminTab === 'jobs' && (
        <div className="admin-grid">
          ...
          <div className="admin-card">
            ...
            {[
              ["案件タイトル *", "title", "text", "例：SaaS新規開拓営業"],
              ["企業名 *", "company", "text", "例：株式会社〇〇"],
              ["報酬 *", "rate", "text", "例：月60〜80万円"],
              ["勤務地", "location", "text", "例：東京（一部リモート）"],
              ["契約期間", "period", "text", "例：長期（6ヶ月〜）"],
              ["タグ（カンマ区切り）", "tags", "text", "例：SaaS, IT, 新規開拓"],
            ].map(([label, key, type, placeholder]) => (
              <div className="admin-form-row" key={key}>
                <label className="admin-label">{label}</label>
                <input className="admin-input" type={type} placeholder={placeholder}
                  value={adminForm[key]} onChange={e => setAdminForm({...adminForm, [key]: e.target.value})} />
              </div>
            ))}
            ...
            <div className="admin-form-row">
              ...
              <textarea className="admin-textarea" placeholder="案件の詳細を入力..."
            ...
            <div className="admin-form-row">
              ...
              <textarea className="admin-textarea" placeholder="必要なスキル・経験..."
                value={adminForm.requirements} onChange={e => setAdminForm({...adminForm, requirements: e.target.value})} />
            </div>
            <div className="admin-form-row">
              <label className="admin-label">フラグ設定</label>
              <div className="admin-checks">
                {[["remote","リモート可"],["urgent","急募"],["highPay","高単価"],["lowExp","経験少なめOK"],["published","公開する"]].map(([key, label]) => (
                  <label key={key} className="admin-check">
                    <input type="checkbox" checked={adminForm[key]} onChange={e => setAdminForm({...adminForm, [key]: e.target.checked})} />
                    {label}
                  </label>
                ))}
              </div>
            </div>
            <button className="admin-btn" onClick={handleAddJob} disabled={adminLoading}>
              {adminLoading ? "保存中..." : editingJob ? "更新する" : "追加する"}
            </button>
          ...
          {/* 案件一覧 */}
          <div className="admin-card">
            <h3>📋 登録済み案件（{jobs.length}件）</h3>
            <div className="admin-job-list">
              {jobs.length === 0 && <div style={{textAlign:'center',color:'var(--gray500)',padding:40}}>まだ案件がありません</div>}
              {jobs.map(job => (
                <div key={job.id} className="admin-job-item">
                  <div className="admin-job-info">
                    <div className="admin-job-title">
                      {job.title}
                      {' '}
                      <span className={`published-badge ${job.published !== false ? 'on' : 'off'}`}>
                        {job.published !== false ? '公開中' : '非公開'}
                      </span>
                    </div>
                    <div className="admin-job-meta">{job.company} ・ {job.rate}</div>
                  </div>
                  <div className="admin-job-actions">
                    <button className={`admin-action-btn ${job.published !== false ? 'unpublish' : 'publish'}`} onClick={() => handleTogglePublish(job)}>
                      {job.published !== false ? '非公開' : '公開'}
                    </button>
      ...
      {adminTab === 'applications' && (
        <div className="admin-card" style={{maxWidth:'100%'}}>
          <h3>📨 応募一覧（{applications.length}件）</h3>
          {applications.length === 0 && <div style={{textAlign:'center',color:'var(--gray500)',padding:40}}>まだ応募がありません</div>}
          {applications.map(app => (
            <div key={app.id} className="app-item" onClick={() => { setSelectedApplication(app); setShowMessages(true); }}>
              <div className="app-item-top">
                <div className="app-item-title">{app.jobTitle}</div>
                <span className={`status-badge ${app.status}`}>{app.status}</span>
              </div>
              <div className="app-item-meta">
                👤 {app.userName}（{app.userEmail}）・ 📅 {app.createdAt?.toDate?.()?.toLocaleDateString?.('ja-JP') || ""}
              </div>
              {app.message && (
                <div style={{fontSize:13,color:'var(--gray700)',marginTop:8,padding:'8px 12px',background:'var(--gray100)',borderRadius:8}}>
                  {app.message.slice(0, 80)}{app.message.length > 80 ? '...' : ''}
                </div>
              )}
              <div style={{display:'flex',gap:8,marginTop:10}} onClick={e => e.stopPropagation()}>
                {["未対応","対応中","採用","不採用"].map(status => (
                  <button key={status}
                    style={{padding:'4px 12px',borderRadius:100,fontSize:11,fontWeight:600,cursor:'pointer',border:'none',
                      background: app.status === status ? 'var(--accent)' : 'var(--gray200)',
                      color: app.status === status ? 'white' : 'var(--gray700)'
                    }}
                    onClick={() => handleUpdateApplicationStatus(app.id, status)}>
                    {status}
                  </button>
                ))}
                <button style={{padding:'4px 12px',borderRadius:100,fontSize:11,fontWeight:600,cursor:'pointer',border:'none',background:'var(--blue-soft)',color:'var(--blue)'}}
                  onClick={() => { setSelectedApplication(app); setShowMessages(true); }}>
                  💬 メッセージ
                </button>
              </div>
        ...
        <div className="msg-overlay" onClick={() => setShowMessages(false)}>
          <div className="msg-panel" onClick={e => e.stopPropagation()}>
            <div className="msg-header">
              <div>
                <div className="msg-header-title">💬 {selectedApplication.jobTitle}</div>
                <div className="msg-header-sub">応募者: {selectedApplication.userName}（{selectedApplication.userEmail}）</div>
              </div>
              <button onClick={() => setShowMessages(false)} style={{background:'none',border:'none',fontSize:20,cursor:'pointer',color:'var(--gray500)'}}>✕</button>
            </div>
            <div className="msg-body">
              ...
              {messages.map(msg => (
                <div key={msg.id} className={`msg-bubble ${msg.isAdmin ? 'mine' : 'theirs'}`}>
                  <div className="msg-bubble-name">{msg.senderName}</div>
                  {msg.text}
                </div>
              ))}
